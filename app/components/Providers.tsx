    ...
    useEffect(() => {
        ...
        const fetchGitHubData = async () => {
            ...
            try {
                ...
                const userResponse = await fetch('/api/github?action=user')
                let startYear = 2018;
                let publicRepos = 0;
                ...
                // 2. Fetch all contributions in one request (last 365 days)
                ...
                const contribResponse = await fetch(`/api/github?action=contributions&from=${from}&to=${to}`)
                ...
                if (contribResponse.ok) {
                    ...
                    if (typeof contribData.lifetimeTotal === 'number') {
                        lifetimeTotal = contribData.lifetimeTotal
                    }
                }

                // Also fetch lifetime stats from user profile if available
                ...
                // 4. Strict 365 Days ending Today
                ...
                setStats({
                    publicRepos,
                    // GitHub's GraphQL total spans the account's full history.
                    // Do not substitute the last-year total when it is unavailable.
                    totalContributions: lifetimeTotal ?? 0,
...
function SystemThemeSync() {
    ...
    useEffect(() => {
        // Always begin in system mode, even if a previous visit used the manual
        // toggle. next-themes then tracks later OS/browser theme changes itself.
        setTheme('system')
